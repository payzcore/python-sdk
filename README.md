...
# Create a payment monitoring request (USDT on TRC20, default)
...
def handle_webhook(request):
    ...
    if event["event"] == "payment.completed":
        ...
        print(f"Amount: {event['paid_amount']} {event['token']} on {event['chain']}")
...
try:
    payment = client.payments.create(
        amount=50, chain="TRC20", external_ref="user-123"
    )
